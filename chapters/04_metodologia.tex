...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    % Flujo automático
    \node[thesisflow] (video) {Videos microscópicos};
    \node[thesisflow, right=of video] (proc) {Procesamiento automático\\Detección y seguimiento};
    \node[thesisflow, right=of proc] (results) {Resultados automáticos\\Trayectorias, métricas y clasificación};

    % Referencia y evaluación
    \node[thesisflow, below=of video] (tracker) {Referencia manual\\Tracker};
    \node[thesisflow, below=of results] (eval) {Comparación y\\evaluación experimental};
    \node[thesisflow, right=of eval] (trace) {Trazabilidad y\\reproducibilidad};

    \draw[thesisarrow] (video) -- (proc);
    \draw[thesisarrow] (proc) -- (results);

    \draw[thesisarrow] (video) -- (tracker);
    \draw[thesisarrow] (tracker) -- (eval);
    \draw[thesisarrow] (results) -- (eval);
    \draw[thesisarrow] (eval) -- (trace);
  \end{tikzpicture}
  \caption{Esquema general del diseño metodológico del estudio, desde el procesamiento automático y la construcción de la referencia manual hasta la evaluación y la trazabilidad de los resultados.}
