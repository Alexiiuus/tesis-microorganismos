...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[thesisflow, below=of video] (proc) {Procesamiento automático\\Detección y seguimiento};
    \node[thesisflow, below=of proc] (results) {Trayectorias, métricas y clasificación};
    \node[thesisflow, below=12mm of results] (eval) {Comparación y evaluación experimental};
    \node[thesisflow, below=of eval] (trace) {Resultados, trazabilidad y reproducibilidad};

    \node[thesisflow, left=17mm of eval, text width=3.2cm] (tracker) {Referencia manual\\Tracker};
  ...
  \end{tikzpicture}
  \caption{Esquema general del diseño metodológico. El procesamiento automático y la referencia manual convergen en la etapa de evaluación, cuyos resultados se registran junto con la información necesaria para su trazabilidad.}
...
\begin{figure}[H]
  ...
  \begin{tabularx}{0.88\textwidth}{@{}>{\raggedright\arraybackslash}p{0.25\textwidth}X@{}}
    \toprule
    \textbf{Material} & \textbf{Función dentro del estudio} \\
    \midrule
    Imágenes anotadas &
    Adaptación y evaluación del detector. \\

    Videos microscópicos &
    Entrada principal del sistema. Cada video constituye una unidad experimental; los microorganismos seguidos son las unidades primarias de observación y sus trayectorias las unidades de análisis computacional. \\

    Trayectorias de Tracker &
    Referencia manual utilizada para comparar y validar las trayectorias obtenidas automáticamente. \\

    Escala micrométrica &
    Referencia empleada para establecer el factor de calibración espacial y convertir las mediciones desde píxeles a unidades físicas. \\
    \bottomrule
  \end{tabularx}
